#!/usr/bin/env python3
"""
add_fy27_market_sheets.py — Add 5 FY2027 market-sizing sheets with live SUMIFS formulas.

Uses a 4-term SUMIFS cascade for FY2027 values (U → S fallback).
Named ranges (JB_27U, JB_27S, etc.) keep formulas readable.
...
WORKBOOK_SRC = 'output/08APR2028_Newbuild_and_MRO_Spend_v2.2.xlsx'
WORKBOOK_DST = 'output/08APR2028_Newbuild_and_MRO_Spend_v2.3.xlsx'
...
# ── FY27 Formula builder ──────────────────────────────────────

def cf(*conditions):
    """4-term SUMIFS cascade for FY2027. Returns '=SUMIFS(...)+...'
    Tries JB_27U (FY2027 Total) first, falls back to JB_27S (FY2027 Request).
    """
    cond = (',' + ','.join(conditions)) if conditions else ''
    t = []
    for rt in ['""', '"[PARENT]"']:
        t.append(f'SUMIFS(JB_27U{cond},JB_S,{rt},JB_27U,"<>")')
        t.append(f'SUMIFS(JB_27S{cond},JB_S,{rt},JB_27U,"",JB_27S,"<>")')
    return '=' + '+'.join(t)
...
def fy27(row):
    """Get FY2027 value: try U (Total) first, then S (Request)."""
    for i in [20, 18]:  # col U (idx 20), col S (idx 18)
        v = row[i].value
...
def create_named_ranges(wb):
    ...
    for nm, col in [('JB_27U', 'U'), ('JB_27S', 'S')]:
        if nm not in wb.defined_names:
            wb.defined_names.add(DefinedName(nm, attr_text=f"{JB}!${col}$5:${col}$3381"))


# ── Sheet 1: FY27 Total Funding ─────────────────────────────

def create_total_funding(wb, d):
    ws = wb.create_sheet('FY27 Total Funding')
    r = 1; wc(ws, r, 1, 'Total Funding', font=TITLE_FONT)
    r = 2; wc(ws, r, 1, 'All Navy + USCG appropriations for vessel-related activity, FY2027 ($K)', font=SUBTITLE_FONT)

    r = 4; wc(ws, r, 1, '(A) FY2027 Total Funding by Work Type', font=SECTION_FONT)
    r = 5; hdr_row(ws, r, [('Work Type', 52), ('FY2027 ($K)', 18)])
    ...
    r += 2; wc(ws, r, 1, '(B) FY2027 Total Funding by Vessel Category', font=SECTION_FONT)
    r += 1; hdr_row(ws, r, [('Category', 52), ('FY2027 ($K)', 18), ('TAM Scope', 28)])
    ...
    r += 2; wc(ws, r, 1, '(C) FY2027 Total Funding by Source Book', font=SECTION_FONT)
    r += 1; hdr_row(ws, r, [('Source Book', 52), ('FY2027 ($K)', 18)]); fs = r + 1
    for s in d['sources']:
...
# ── Sheet 2: FY27 Newbuild TAM ──────────────────────────────

def create_newbuild_tam(wb, d):
    ws = wb.create_sheet('FY27 Newbuild TAM')
    r = 1; wc(ws, r, 1, 'Newbuild Total Addressable Market \u2014 FY2027 ($K)', font=TITLE_FONT)
    ...
    r += 1; hdr_row(ws, r, [('Vessel Type', 40), ('Service', 10), ('FY2027 ($K)', 16), ('% of TAM', 12)])
    ft = r + 1
    for vt in all_types:
...
# ── Sheet 3: FY27 Newbuild SAM ──────────────────────────────

def create_newbuild_sam(wb, d):
    ws = wb.create_sheet('FY27 Newbuild SAM')
    r = 1; wc(ws, r, 1, 'Newbuild Serviceable Addressable Market \u2014 FY2027 ($K)', font=TITLE_FONT)
    ...
    r += 1; hdr_row(ws, r, [('Vessel Type', 40), ('Service', 10), ('FY2027 ($K)', 16), ('% of SAM', 12)])
    ft = r + 1
    for vt in all_types:
        r += 1; wc(ws, r, 1, vt, font=DATA_FONT)
        wc(ws, r, 2, d['type_svc'].get(vt, ''), font=DATA_FONT)
...
# ── Sheet 4: FY27 MRO TAM ───────────────────────────────────

def create_mro_tam(wb, d):
    ws = wb.create_sheet('FY27 MRO TAM')
    r = 1; wc(ws, r, 1, 'MRO Total Addressable Market \u2014 FY2027 ($K)', font=TITLE_FONT)
    ...
    r += 1; hdr_row(ws, r, [('Work Type', 52), ('FY2027 ($K)', 16), ('% of TAM', 12)])
    fb = r + 1
    for b in bkts:
...
# ── Sheet 5: FY27 MRO SAM ───────────────────────────────────

def create_mro_sam(wb, d, mro_tam_info=None):
    ws = wb.create_sheet('FY27 MRO SAM')
    r = 1; wc(ws, r, 1, 'MRO Serviceable Addressable Market \u2014 FY2027 ($K)', font=TITLE_FONT)
    ...
    wc(ws, r, 2, f"='FY27 MRO TAM'!B{tam_ref_row}", font=DATA_FONT, fmt=NUM_FMT)
...
def main():
    print('Reading J Book data (FY2027 values)...')
    d = read_data()
    ...
    # Remove pre-existing FY27 market sheets so we don't create duplicates
    for nm in ['FY27 Total Funding', 'FY27 Newbuild TAM', 'FY27 Newbuild SAM', 'FY27 MRO TAM', 'FY27 MRO SAM']:
        if nm in wb.sheetnames:
            del wb[nm]

    print('Creating FY27 named ranges...')
    create_named_ranges(wb)

    print('Creating FY27 sheets...')
    ...
    # Place FY27 sheets after FY26 MRO SAM, before Competitive Dynamics
    fy27_sheets = ['FY27 Total Funding', 'FY27 Newbuild TAM', 'FY27 Newbuild SAM', 'FY27 MRO TAM', 'FY27 MRO SAM']
    other = [s for s in wb.sheetnames if s not in fy27_sheets]

    # Find insertion point: after 'FY26 MRO SAM'
    if 'FY26 MRO SAM' in other:
        idx = other.index('FY26 MRO SAM') + 1
        ordered = other[:idx] + fy27_sheets + other[idx:]
    else:
        # Fallback: put after first sheet
        ordered = [other[0]] + fy27_sheets + other[1:]
    ...
    # Delete v2.2 source
    if os.path.exists(WORKBOOK_SRC) and WORKBOOK_SRC != WORKBOOK_DST:
        os.remove(WORKBOOK_SRC)
        print(f'Deleted {WORKBOOK_SRC}')

    # Verify no formula exceeds limit
    wb2 = openpyxl.load_workbook(WORKBOOK_DST)
    max_len = 0; max_cell = ''
    for nm in fy27_sheets:
