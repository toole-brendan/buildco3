...
def populate_derived_columns(wb):
    """Populate Exhibit Level (col 24) and Cost Category (col 25) for ALT_VIEW rows.
    These columns are inserted between Parent Item and Vessel Service by the build script."""
    ws = wb['J Book Items Cons.']
    ...
    ws.cell(row=4, column=24, value='Exhibit Level')
    ws.cell(row=4, column=25, value='Cost Category')
...
def update_validation_sheet(wb):
    ...
    vrow('J Book Items Cons.', 'Column', 'Exhibit Level (col X)',
         'Derived column inserted by the build script between Parent Item and Vessel Service. '
         'Populated for [ALT_VIEW] rows. '
    ...
    vrow('J Book Items Cons.', 'Column', 'Cost Category (col Y)',
         'Derived column inserted by the build script between Parent Item and Vessel Service. '
         'Populated for [ALT_VIEW] rows. '
    ...
    vrow('Newbuild SAM Cost Detail', 'Section', '(B) P-8a System-Level Detail',
         ...
         'Cost Element column (col AJ) provides the system name; '
    ...
    vrow('J Book Items Cons.', 'Named Range', 'JB_CE (col AJ)',
         'Cost Element — existing column containing system names for P-8a rows '
         '(e.g., "AN/SPY-6(V)1 (AMDR)"). Shifted +2 cols due to inserted derived columns.',
         'Range: $AJ$5:$AJ$3381. Populated for ~385 of 415 P-8a rows.')

    vrow('J Book Items Cons.', 'Named Range', 'JB_EX (col X)',
         'Exhibit Level — derived column inserted between Parent Item and Vessel Service. '
         'Values: "P-5c", "P-8a", "P-35", or blank.',
         'Range: $X$5:$X$3381. Used in SUMIFS to filter by exhibit type.')

    vrow('J Book Items Cons.', 'Named Range', 'JB_CC (col Y)',
         'Cost Category — derived column inserted between Parent Item and Vessel Service. '
         'Values: P-5c cost category name or blank.',
         'Range: $Y$5:$Y$3381. Used in SUMIFS to filter by cost category.')
...
def create_named_ranges(wb):
    ...
    shared = [
        ...
        ('JB_S', 'V'), ('JB_U', 'Z'), ('JB_V', 'AA'), ('JB_W', 'AB'),
        ('JB_CE', 'AJ'),   # Cost Element (shifted: was col AH, +2 for inserted cols)
        ('JB_EX', 'X'),    # Exhibit Level (inserted col 24)
        ('JB_CC', 'Y'),    # Cost Category (inserted col 25)
    ]
    fy_ranges = []
...
def main():
    ...
    # Insert 2 columns at position 24 (between Parent Item and Vessel Service)
    # so derived columns (Exhibit Level, Cost Category) sit right after Parent Item.
    print('Inserting derived columns at col X (between Parent Item and Vessel Service)...')
    wb['J Book Items Cons.'].insert_cols(24, 2)

    # Named ranges (must come after column insertion so letters match)
